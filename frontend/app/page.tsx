      ...
      <div className={`relative z-10 ${isWarping ? 'animate-warp' : ''}`}>
        <div className="max-w-7xl mx-auto px-8 md:px-16 pt-8 md:pt-16">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center min-h-[60vh]">
            ...
            >
              ...
              >
                Academic papers visualized in 3D space.
                Citation topology, semantic clusters, temporal depth
              ...
              >
                {[
                  { number: '16M+', label: 'citations indexed' },
                  { number: '200+', label: 'papers per graph' },
        ...
        >
          ...
          <div className="flex items-center gap-1 mb-4">
            {([
              { mode: 'doi' as InputMode, label: 'Seed Paper' },
              { mode: 'natural' as InputMode, label: 'Search' },
            ]).map(({ mode, label }) => (
              <button
                key={mode}
                ...
                <form onSubmit={handleSubmit}>
                  <div className="flex items-stretch border border-neutral-800 rounded-lg overflow-hidden bg-neutral-950 hover:border-neutral-700 focus-within:border-[#D4AF37]/40 focus-within:shadow-[0_0_20px_rgba(212,175,55,0.08)] transition-all duration-300">
                    <input
                      ...
                      placeholder="Enter a DOI, URL, or arXiv link..."
              ...
              <motion.div key="natural" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                ...
                <AnimatePresence>
                  {naturalResults && naturalResults.length > 0 && (
                    ...
                    >
                      <p className="text-[10px] text-neutral-600 px-1 font-mono uppercase tracking-wider mb-1">
                        {naturalResults.length} papers found:
                      </p>
                      {naturalResults.map((paper: any) => (
                        <button
                          ...
                          className="text-left rounded-lg p-4 bg-neutral-950 border border-neutral-800 hover:border-neutral-600 transition-all group"
                        >
                          <h3 className="text-sm font-medium text-white group-hover:text-[#D4AF37] transition-colors leading-snug">
                            {paper.title}
                          </h3>
